...
export default function Login() {
  ...
  const { signInWithEmail, signInWithGoogle, authLoading, authError } = useRelay();
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      setError(err?.message || "Google login was interrupted");
    }
  }
        ...
        <GlassCard className="p-6 sm:p-8">
          <div className="text-center mb-6">
            ...
            <p className="mt-1.5 text-xs text-white/52">
              Sign in to manage your local exchanges
            </p>
          </div>
          ...
          <p className="mt-6 text-center text-xs text-white/44">
            New to Relay?{" "}
            <Link to="/signup" className="text-[#8dcde4] hover:underline font-semibold">
              Create an account
            </Link>
          </p>
        </GlassCard>
